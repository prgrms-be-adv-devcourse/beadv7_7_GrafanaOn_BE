...
// 측정 대상: PATCH /api/offers/{offerId}/accept
//
...
//    수락 시 상품도 판매완료로 바뀌어 상품까지 소모된다.
//
//    OFFER_IDS 환경변수로 처리할 오퍼 목록을 넘긴다.

export const options = singleOptions('PATCH /api/offers/:id/accept');
...
export default function (data) {
    ...
    const res = http.patch(
        `${COMMERCE_BASE_URL}/api/offers/${offerId}/accept`,
        null,
        { headers: authHeaders(seller), tags: { name: 'PATCH /api/offers/:id/accept' } }
    );

    check(res, { '오퍼 수락 200': (r) => r.status === 200 });
    sleep(THINK_TIME);
}
